...
public class MainSettingsActivity extends PreferenceActivity {
	...
	private void setupSimplePreferencesScreen(){
		...
		// In the simplified UI, fragments are not used at all and
		// we instead
		// use the older PreferenceActivity APIs.

		// Add preferences categories from resources.
		addPreferencesFromResource(R.xml.pref_notifications);
		addPreferencesFromResource(R.xml.pref_general);

		// Bind the summaries of EditText/List/Dialog/Ringtone
		// preferences to
		// their values. When their values change, their summaries
		// are updated
	 ...
	 * device doesn't have newer APIs like {@link android.preference.PreferenceFragment}, or
	...
	public void onBuildHeaders(List<Header> target){
		...
		if(!isSimplePreferences(this)){// || deviceOnLandscape(this)){
	...
	private static Preference.OnPreferenceChangeListener sBindPreferenceSummaryToValueListener = new Preference.OnPreferenceChangeListener() {
		...
		public boolean onPreferenceChange(Preference preference,
			...
			if(preference instanceof EditTextPreference){	
				...
				int index = list.findIndexOfValue(stringValue);
				
				list.setSummary((index >= 0) ? list.getEntries()[index] : null);
			}
	 ...
	 * the preference title) is updated to reflect the value. The summary
	 * is also immediately updated upon calling this method. The exact
	...
	PreferenceFragment {
		...
		public void onCreate(Bundle savedInstanceState){
			...
			// to their values. When their values change,
			// their summaries are
			// updated to reflect the new value, per the
			// Android Design
			// guidelines.
			bindPreferenceSummaryToValue(findPreference(LINK));
			bindPreferenceSummaryToValue(findPreference(LIST_LED_COLOR));
